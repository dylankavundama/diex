    ...
    if (!empty($nom) && !empty($telephone)) {
        ...
        if ($existing) {
            ...
            if ($insert_client->execute()) {
                ...
                header('Location: create_sale.php?client_id=' . $new_client_id);
                exit();
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_order') {
    ...
    $reduction = (float)($_POST['reduction'] ?? 0); // Montant de réduction en USD
    ...
    if (!empty($products) && is_array($products)) {
        ...
        try {
            ...
            if ($reduction > 0 && $reduction <= $total) {
                ...
                throw new Exception('La réduction ne peut pas être supérieure au total.');
            }
            
            // Générer un numéro de commande unique
            $numero_commande = 'CMD-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -6));
            
            // Créer la commande (on stocke le total après réduction, et la réduction dans les notes si > 0)
            $notes_with_reduction = $notes;
            if ($reduction > 0) {
                $notes_with_reduction = ($notes ? $notes . "\n\n" : '') . "Réduction appliquée: " . formatPrice($reduction) . " (" . formatPriceCdf($reduction) . ")";
            }
            ...
            $order_stmt->bind_param("isdss", $client_id, $numero_commande, $total, $mode_paiement, $notes_with_reduction);
            $order_stmt->execute();
            ...
            // Gérer le paiement selon le type de vente (utiliser le total après réduction)
            if ($type_vente === 'credit') {
                ...
                if ($reduction > 0) {
                    $payment_description .= " (Réduction: " . formatPrice($reduction) . ")";
                }
...
<div class="content-card">
    <form method="POST" id="orderForm">
        ...
        <div class="form-group">
            <label>Produits * <small style="color: #7f8c8d;">(Vous pouvez sélectionner plusieurs produits)</small></label>
            <div id="productsList" style="border: 1px solid #ddd; padding: 1rem; border-radius: 5px; max-height: 500px; overflow-y: auto;">
                <?php while ($product = $products_list->fetch_assoc()): ?>
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; border-bottom: 1px solid #ddd; transition: background-color 0.2s; gap: 1rem;" onmouseover="this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor=''">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <small style="color: #7f8c8d; display: block;">
                                Stock: <?php echo $product['stock']; ?> | Prix unitaire: <?php echo formatPriceDual($product['prix_vente']); ?>
                            </small>
                        </div>
        ...
        <div class="form-group">
            <label for="reduction">Réduction (USD) <small style="color: #7f8c8d;">(optionnel)</small></label>
            <input type="number" id="reduction" name="reduction" class="form-control" step="0.01" min="0" value="0" placeholder="0.00">
            <small class="form-text text-muted">Montant de réduction à appliquer sur le total. La réduction sera indiquée sur la facture.</small>
        </div>
        ...
        <div style="background: #f8f9fa; padding: 1rem; border-radius: 5px; margin: 1rem 0;">
            <div id="sousTotalDisplay" style="font-size: 1rem; color: #7f8c8d; text-align: right; margin-bottom: 0.5rem;">
                Sous-total: $0.00 (0 CDF)
            </div>
            <div id="reductionDisplay" style="font-size: 1rem; color: #27ae60; text-align: right; margin-bottom: 0.5rem; display: none;">
                Réduction: -$0.00 (0 CDF)
            </div>
            <div id="totalDisplay" style="font-size: 1.5rem; font-weight: bold; color: #e74c3c; text-align: right; border-top: 2px solid #ddd; padding-top: 0.5rem;">
                Total: $0.00 (0 CDF)
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotal() {
        ...
        const sousTotalCdf = sousTotal * USD_TO_CDF_RATE;
        sousTotalDisplay.textContent = 'Sous-total: $' + sousTotal.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + 
                                       ' (' + sousTotalCdf.toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0}) + ' CDF)';
        
        // Afficher la réduction si elle existe
        if (reduction > 0) {
            const reductionCdf = reduction * USD_TO_CDF_RATE;
            reductionDisplay.style.display = 'block';
            reductionDisplay.textContent = 'Réduction: -$' + reduction.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + 
                                          ' (-' + reductionCdf.toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0}) + ' CDF)';
        } else {
            reductionDisplay.style.display = 'none';
        }
        
        // Afficher le total final
        const totalCdf = total * USD_TO_CDF_RATE;
    ...
    // Ajouter l'écouteur pour le champ de réduction
    const reductionInput = document.getElementById('reduction');
    reductionInput.addEventListener('input', function() {
        const sousTotal = parseFloat(sousTotalDisplay.textContent.match(/\$([\d,]+\.\d{2})/)?.[1]?.replace(/,/g, '') || 0);
        const reduction = parseFloat(this.value) || 0;
        
        if (reduction > sousTotal) {
            this.setCustomValidity('La réduction ne peut pas être supérieure au sous-total.');
